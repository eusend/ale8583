defmodule Ale8583 do
  ...
  @spec new({any(), any()}) ::
          {:error, [], <<_::704>>}
          | {:error, [], {}, <<_::64, _::_*8>> | {:error, <<_::64, _::_*8>>}}
          | {:iso,
             [{:bp, <<_::128>>} | {:bs, <<_::128>>} | {:header_prosa, <<>>} | {:mti, any()}, ...],
             {[any()], [any()], :flag_bm_sec_no, any()}, {:ok, <<_::16>>}}
      ...
      type in [:prosa, :mc] ->
        iso =
          if type == :prosa do
            ...
            list_iso =
              List.keyreplace(list_iso, :header_prosa, 0, {:header_prosa, str_header_prosa})
  ...
  defp new(:conf, {mti, list_conf}, headerProsa) do
    case mti in @msgs do
      true ->
        {:iso,
         [mti: mti, bp: "0000000000000000", bs: "0000000000000000", header_prosa: headerProsa],
          ...
          true ->
            {:iso,
             [
               ...
               bp: "0000000000000000",
               bs: "0000000000000000",
               header_prosa: headerProsa
             ],
             {Ale8583.BitMap.newBitMapP(), Ale8583.BitMap.newBitMapS(), :flag_bm_sec_no,
              list_conf}, {:ok, "OK"}}
  ...
  defp do_printAll([head | tail], list_iso_conf, buffer) do
    {key, value} = head
    # Logger.warn("key #{inspect(key)}, value #{inspect(value)}")
    ...
    buffer =
      if key in [:mti, :bp, :bs, :header_prosa] do
        ...
        case map.var_length do
          ...
          # buffer <> " #{key}<#{String.slice(value, 0..(map.length + 1))}>\n"
      ...
      false ->
        ...
        cond do
          key in 1..64 ->
            ...
            {:iso, list_iso, {list_bit_map_p, list_bit_map_s, flag_bm_sec, list_iso_conf},
             {:ok, "OK INSERT AND PRIMARY BITMAP UPDATE,#{atom_key}-#{key}: value #{value}"}}
          ...
          key in 65..128 ->
            {list_bit_map_p, flag_bm_sec, list_iso} =
              if flag_bm_sec == :flag_bm_sec_no do
                ...
                {list_bit_map_p_temp, :flag_bm_sec_yes, list_iso_temp}
              else
                {list_bit_map_p, flag_bm_sec, list_iso}
              end
            ...
            {:iso,
             ...
             {:ok, "OK INSERT AND SECUNDARY BITMAP UPDATE,#{atom_key}-#{key}: value #{value}"}}
